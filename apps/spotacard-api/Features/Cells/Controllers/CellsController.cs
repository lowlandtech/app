...
namespace Spotacard.Features.Cells.Controllers
{
    [Route("cells")]
    public class CellsController : Controller
    {
        ...
        [HttpGet]
        public async Task<CellsEnvelope> Get([FromQuery] string author, [FromQuery] int? limit, [FromQuery] int? offset)
        {
            return await _mediator.Send(new List.Query(author, limit, offset));
        }

        [HttpGet("{id}")]
        public async Task<CellEnvelope> Get(Guid id)
        {
        ...
        public async Task<CellEnvelope> Create([FromBody] Create.Command command)
        {
        ...
        [HttpPut("{id}")]
        [Authorize(AuthenticationSchemes = JwtIssuerOptions.Schemes)]
        public async Task<CellEnvelope> Edit(Guid id, [FromBody] Edit.Command command)
        {
            command.CellId = id;
            return await _mediator.Send(command);
        }
